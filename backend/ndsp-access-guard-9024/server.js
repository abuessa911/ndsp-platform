...
const PORT = Number(process.env.PORT || 9024);
...
const env = Object.assign(
  ...
  readEnvFile("/home/nawaf511/.config/ndsp/backend.env"),
  process.env
);
  ...
  try {
    ...
    if (url.pathname === "/" || url.pathname === "/health") {
      try {
        ...
        return send(res, 200, { ok:true, service:"ndsp-access-guard-9024", port:PORT, database:"ok" });
      } catch (e) {
        return send(res, 200, { ok:true, service:"ndsp-access-guard-9024", port:PORT, database:"review", message:String(e.message || e) });
      }
    }
...
server.on("error", (e) => {
  console.error("ndsp-access-guard-9024 server error:", e);
  process.exit(1);
});

server.listen(PORT, "127.0.0.1", () => {
  console.log("ndsp-access-guard-9024 listening on", PORT);
});
