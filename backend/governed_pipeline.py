...
def _ndsp_attach_tdl_v2_safe(decision_payload: dict) -> dict:
    """
    Safely attach TDL v2 to the final governed decision payload.
    If TDL v2 fails for any reason, preserve the original payload.
    """
    try:
...
def _tdl_primary_direction(tdl: dict) -> str:
    return _direction(tdl.get("tdl_lm_direction") or tdl.get("dominant_side") or "neutral")
...
def _get_timing_v4() -> dict:
    ...
    return {
        "active": f"TDL-{controller}" if controller != "NEUTRAL" else "OFF",
        "controller": controller,
        "day_group": day_group,
        "timestamp": now.isoformat(),
    }

def _resolve_dominant_direction(timing: dict, tdl: dict) -> dict:
    """
    ...
    # محاولة سحب الاتجاه من الهيكلة القديمة لـ TDL إذا لم تكن الهيكلة الجديدة مطبقة بالكامل
    tdl_lm = _direction(tdl.get("tdl_lm_direction", tdl.get("weekly", {}).get("lm_direction", "neutral")))
    tdl_s = _direction(tdl.get("tdl_s_direction", tdl.get("weekly", {}).get("s_direction", "neutral")))
...
def _phase_from_tdl(tdl: dict) -> dict:
    lm = str(tdl.get("tdl_lm_direction") or "").upper()
    delta = str(tdl.get("tdl_lm_delta") or "").upper()
...
def _run_momentum_dual_safe(symbol: str, market: dict, tdl: dict) -> dict:
    try:
        result = _safe_dict(run_momentum_dual(symbol=symbol, market=market, tdl=tdl))
    except TypeError:
    ...
    tdl_dir = _tdl_primary_direction(tdl)
...
def _run_nmp_safe(symbol, market, phase, timing):
    return evaluate_nmp_context(symbol=symbol, market=market, tdl={}, phase=phase, timing=timing)
...
def _run_black_layer_safe(symbol: str, market: dict, momentum_dual: dict, phase: dict, timing: dict) -> dict:
    try:
        ...
        return {
            ...
            "public_note": "Black Layer refined protective context without defining direction.",
        }
    except Exception:
        return {"state": "neutral_context", "score": 0.0, "confidence_effect": "neutral", "risk_effect": "normal", "context": "protective filtering unavailable; decision continued safely", "public_note": "Black Layer unavailable; governed pipeline continued safely."}

def _compute_decision_quality_stack(tdl: dict, conflict: dict, momentum_dual: dict, black_layer: dict, timing: dict) -> dict:
    """
    ...
    lm_dir = _direction(tdl.get("tdl_lm_direction", "neutral"))
    s_dir = _direction(tdl.get("tdl_s_direction", "neutral"))
...
def _run_governed_raw(symbol: str, user: dict | None = None, manual_tdl: dict | None = None) -> dict:
    ...
    try:
        ...
        tdl = _safe_dict(run_tdl(symbol=symbol, data=tdl_data, profile=profile))
    except Exception:
        tdl = {"tdl_lm_direction": "NEUTRAL", "tdl_lm_delta": "NEUTRAL", "tdl_s_direction": "NEUTRAL", "tdl_s_delta": "NEUTRAL", "dominant_side": "NEUTRAL"}

    phase = _phase_from_tdl(tdl)
    timing = _get_timing_v4() # V4 Timing
    momentum_dual = _run_momentum_dual_safe(symbol=symbol, market=market, tdl=tdl)
    ...
    nmp = _run_nmp_safe(symbol=symbol, market=market, phase=phase, timing=timing)
    intelligence["nmp_tdl_quality"] = evaluate_nmp_tdl_quality(nmp, tdl, timing=timing)

    try: conflict = _safe_dict(detect_conflict({"tdl": tdl, "phase": phase, "intelligence": intelligence}))
    ...
    # Layer 6: Direction ONLY from Timing + TDL
    dominant_direction = _resolve_dominant_direction(timing, tdl)
    ...
    quality_stack = _compute_decision_quality_stack(tdl, conflict, momentum_dual, black_layer, timing)
    ...
    governed = {
        ...
        "scenario": {
            ...
            "invalidation": "invalidated if TDL structure breaks or risk enters safe mode",
        ...
        "explainability": {
            ...
            "context_explanation": "Direction resolved exclusively from Dominant Timed Direction (Timing + TDL).",
