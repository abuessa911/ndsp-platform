...
تحديد المسار الصحيح تلقائياً

if [ -f "governed_pipeline.py" ]; then
CORE_DIR="."
elif [ -d "app/core" ]; then
CORE_DIR="app/core"
else
echo -e "${RED}❌ الرجاء تشغيل السكريبت من مجلد backend أو app/core.${NC}"
exit 1
fi

BACKUP_DIR="${CORE_DIR}_backup_V4_$(date +%Y%m%d_%H%M%S)"
...
echo -e "${YELLOW}تم تجميع الملفات في: ${NC}$OUTPUT_BATCH"
