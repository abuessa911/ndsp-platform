...
router = APIRouter(prefix="/api/ndsp/internal/telegram-routing-v2", tags=["telegram-routing-internal-v2"])
...
def require_admin(
    ...
    key = pick_key(x_admin_key, x_admin_token, x_ndsp_admin_key, x_ndsp_admin_action_key, authorization)
    if not admin_key_ok(key):
        raise HTTPException(status_code=401, detail="UNAUTHORIZED_INTERNAL_TELEGRAM_ROUTING")

def _clean(v: str) -> str:
    return (v or "").strip().strip('"').strip("'")

def db_conn():
    # Prefer explicit DB URL if valid.
    dsn = _clean(env("DATABASE_URL") or env("NDSP_DATABASE_URL"))
    if dsn and "://" in dsn:
        return psycopg2.connect(dsn)

    # Official fallback for this project.
    dbname = _clean(env("POSTGRES_DB")) or "ndsp_auth"
    user = _clean(env("POSTGRES_USER")) or "postgres"
    host = _clean(env("POSTGRES_HOST")) or ""

    # If no host is configured, use local socket first. This avoids TCP password mismatch.
    port = _clean(env("POSTGRES_PORT")) or "5432"
    password = _clean(env("POSTGRES_PASSWORD"))
...
def db_debug_safe():
    return {
        "DATABASE_URL_present": bool(_clean(env("DATABASE_URL"))),
        "NDSP_DATABASE_URL_present": bool(_clean(env("NDSP_DATABASE_URL"))),
        "POSTGRES_DB": _clean(env("POSTGRES_DB")) or "ndsp_auth",
        "POSTGRES_USER": _clean(env("POSTGRES_USER")) or "postgres",
        "POSTGRES_HOST_present": bool(_clean(env("POSTGRES_HOST"))),
        "POSTGRES_PORT": _clean(env("POSTGRES_PORT")) or "5432",
        "POSTGRES_PASSWORD_present": bool(_clean(env("POSTGRES_PASSWORD"))),
    }
    ...
    try:
        with db_conn() as con:
            with con.cursor() as cur:
                ...
                row = cur.fetchone()
        return {"ok": True, "db": db_debug_safe(), "current_database": row[0], "current_user": row[1], "port": row[2]}
    except Exception as exc:
        return {"ok": False, "error": "DB_CONNECT_FAILED", "message": str(exc), "db": db_debug_safe()}
...
def status(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail={"error": "TELEGRAM_STATUS_QUERY_FAILED", "message": str(exc), "db": db_debug_safe()})
...
def update(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail={"error": "TELEGRAM_UPDATE_FAILED", "message": str(exc), "db": db_debug_safe()})
