...
echo "🚀 جاري بناء مُستهلك السيناريو والتفسير (Layer 16)..."

# 1. كتابة كود Scenario Consumer
cat << 'INNER_EOF' > src/eventbus/consumers/ScenarioConsumer.ts
...
# 2. كتابة ملف تشغيل المستهلك
cat << 'INNER_EOF' > src/test_scenario.ts
...
echo "✅ تم إنشاء ScenarioConsumer.ts وملف التشغيل test_scenario.ts بنجاح."
