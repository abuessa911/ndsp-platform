 ...
 try:
  ...
  if kind in {'json','decision'}:
   ...
   if kind=='decision':
    ...
    governed_withheld=(
     isinstance(score,dict)
     and score.get('contract_version')=='NDSP_COMMERCIAL_SCORE_V202'
     and score.get('status')=='WITHHELD_GOVERNANCE_GATE'
     and score.get('public_allowed') is False
     and score.get('decision_authority') is False
     and score.get('not_recommendation') is True
     and score.get('no_buy_sell') is True
     and isinstance(score.get('gates'),dict)
     and isinstance(score.get('missing_components'),list)
     and isinstance(score.get('blockers_count'),int)
    )
    score_ok=numeric_score or governed_withheld
    ok=ok and len(layers)==16 and len(caps)==28 and score_ok
    detail.update({
     'layers':len(layers),
     'capabilities':len(caps),
     'commercial_score_ok':score_ok,
     'commercial_score_status':score.get('status') if isinstance(score,dict) else None,
     'commercial_score_numeric':numeric_score,
     'commercial_score_governed_withheld':governed_withheld
    })
  return {'ok':ok,**detail}
