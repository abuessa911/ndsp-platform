...
This report captures canonical-source evidence without changing production. No file is final until the owner approves one source path per responsibility.
...
Evidence and candidate sources were captured. Do not merge, delete, or rebuild until one path per layer and runtime responsibility is approved.
