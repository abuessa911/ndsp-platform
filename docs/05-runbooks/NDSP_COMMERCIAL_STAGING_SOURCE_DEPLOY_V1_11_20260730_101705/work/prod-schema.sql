...
\restrict SDeWJjCOxaD147DAOao0Kw6Rh6LHVSW0ffCKSzrqfNAy3eWgGxVZSsE3Zn5ikbO
...
--
-- Name: ndsp_guard; Type: SCHEMA; Schema: -; Owner: -
--

CREATE SCHEMA ndsp_guard;
...
--
-- Name: norm_email(text); Type: FUNCTION; Schema: ndsp_guard; Owner: -
--

CREATE FUNCTION ndsp_guard.norm_email(v text) RETURNS text
...
--
-- Name: norm_phone(text); Type: FUNCTION; Schema: ndsp_guard; Owner: -
--

CREATE FUNCTION ndsp_guard.norm_phone(v text) RETURNS text
    LANGUAGE sql IMMUTABLE
    AS $$
...
--
-- Name: prevent_duplicate_identity(); Type: FUNCTION; Schema: ndsp_guard; Owner: -
--

CREATE FUNCTION ndsp_guard.prevent_duplicate_identity() RETURNS trigger
    LANGUAGE plpgsql
    AS $_$
DECLARE
  email_col text := TG_ARGV[0];
  phone_col text := TG_ARGV[1];
  new_email text := '';
  new_phone text := '';
  q text;
  found_count int := 0;
BEGIN
  IF email_col IS NOT NULL AND email_col <> '' THEN
    EXECUTE format('SELECT ndsp_guard.norm_email(($1).%I::text)', email_col)
    INTO new_email
    USING NEW;
  END IF;

  IF phone_col IS NOT NULL AND phone_col <> '' THEN
    EXECUTE format('SELECT ndsp_guard.norm_phone(($1).%I::text)', phone_col)
    INTO new_phone
    USING NEW;
  END IF;
  ...
  IF length(new_email) > 0 THEN
    q := format(
      'SELECT count(*) FROM %I.%I WHERE ndsp_guard.norm_email(%I::text) = $1',
      TG_TABLE_SCHEMA, TG_TABLE_NAME, email_col
    );

    IF TG_OP = 'UPDATE' THEN
      q := q || ' AND ctid <> $2';
      EXECUTE q INTO found_count USING new_email, OLD.ctid;
    ELSE
      EXECUTE q INTO found_count USING new_email;
    END IF;

    IF found_count > 0 THEN
      RAISE EXCEPTION 'DUPLICATE_EMAIL_BLOCKED';
    END IF;
  END IF;

  IF length(new_phone) > 0 THEN
    q := format(
      'SELECT count(*) FROM %I.%I WHERE ndsp_guard.norm_phone(%I::text) = $1',
      TG_TABLE_SCHEMA, TG_TABLE_NAME, phone_col
    );

    IF TG_OP = 'UPDATE' THEN
      q := q || ' AND ctid <> $2';
      EXECUTE q INTO found_count USING new_phone, OLD.ctid;
    ELSE
      EXECUTE q INTO found_count USING new_phone;
    END IF;

    IF found_count > 0 THEN
      RAISE EXCEPTION 'DUPLICATE_PHONE_BLOCKED';
    END IF;
  END IF;
...
--
-- Name: prevent_duplicate_identity_safe(); Type: FUNCTION; Schema: ndsp_guard; Owner: -
--

CREATE FUNCTION ndsp_guard.prevent_duplicate_identity_safe() RETURNS trigger
    LANGUAGE plpgsql
    AS $_$
DECLARE
  email_col text := TG_ARGV[0];
...
\unrestrict SDeWJjCOxaD147DAOao0Kw6Rh6LHVSW0ffCKSzrqfNAy3eWgGxVZSsE3Zn5ikbO
