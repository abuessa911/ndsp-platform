...
def selector_bound(scripts: str, attrs: dict, text: str) -> bool:
    ...
    if idv in {"loginBtn", "verifyBtn", "backBtn"} and "ndsp-login-buttons-binding-js" in scripts:
        return True

    action_words = ["عرض", "view", "تفعيل", "activate", "تعليق", "suspend", "حظر", "block", "اعتماد", "confirm", "approve", "رفض", "reject"]
    if any(w.lower() in t for w in action_words) and "ndsp-admin-action-buttons-binding-js" in scripts:
        return True
    if cls in {"ok", "bad"} and "ndsp-admin-action-buttons-binding-js" in scripts:
        return True
    ...
    for item in parser.items:
        ...
        if tag == "a":
            if href.lower() in DEAD_HREFS:
                if js_binds_id(scripts, idv) or selector_bound(scripts, attrs, text):
                    continue
