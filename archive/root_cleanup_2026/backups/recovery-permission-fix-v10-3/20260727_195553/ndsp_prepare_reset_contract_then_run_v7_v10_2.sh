...
V7_SCRIPT="${NDSP_V7_SCRIPT:-$HOME/ndsp_auth_recovery_true_source_fix_v7_1.sh}"

EXPECTED_RECOVERED_SHA="a4c7e7ecb8dd0e3cca396f042657f8ec51b90fac0a45d6f6b5f85ff28034b90a"
EXPECTED_V7_SHA="6d6bdee37f502068fad53e093712c290e1232cc5630b7548b3a4861953953bdd"
...
# NDSP_RECOVERY_STRUCTURAL_GATE_V10_2
for RECOVERY_FILE in "$FORGOT_HTML" "$RECOVERED_RESET"; do
    if grep -Eqi \
        ...
        fail "RECOVERY_SOURCE_CONTAINS_PORTAL_STRUCTURE:$RECOVERY_FILE"
    fi
done
...
grep -Fq '/ndsp-rp/reset' "$RECOVERED_RESET" ||
    grep -Fq '/api/auth/reset-password' "$RECOVERED_RESET" ||
    fail "RECOVERED_RESET_ENDPOINT_NOT_RECOGNIZED"

grep -Eqi 'newPassword|new_password|confirmPassword|newPassword2' "$RECOVERED_RESET" ||
    fail "RECOVERED_RESET_PASSWORD_FIELDS_MISSING"
