#!/usr/bin/env python3
"""Validate PR-042 batch-04 evidence closure."""
...
PR042 = ROOT / "docs/99-governance/pr-042-p0-batch-04-evidence-closure"


def fail(message: str) -> int:
    print(f"error={message}", file=sys.stderr)
    print("validation=FAIL", file=sys.stderr)
    print("status=PR042_VALIDATION_FAILED", file=sys.stderr)
    return 1
...
def main() -> int:
    batch_input = rows(PR042 / "PR042_BATCH_INPUT.csv")
    evidence = rows(PR042 / "PR042_EVIDENCE_MATRIX.csv")
    results = rows(PR042 / "PR042_CLOSURE_RESULTS.csv")
    remaining = rows(PR042 / "PR042_REMAINING_GAPS.csv")
    summary = json.loads(
        (PR042 / "PR042_SUMMARY.json").read_text(encoding="utf-8")
    )
    ...
    if summary["ui_complete_records_created"] != 0:
        return fail("PR-042 must not create UI_COMPLETE records")

    if summary["full_capability_coverage_claimed"] is not False:
        return fail("PR-042 must not claim full capability coverage")
    ...
    print("status=PR042_P0_BATCH_04_EVIDENCE_CLOSURE_VALID")
    return 0
