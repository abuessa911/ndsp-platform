#!/usr/bin/env python3
"""Validate PR-038 batch-02 evidence closure."""
...
PR038 = ROOT / "docs/99-governance/pr-038-p0-batch-02-evidence-closure"


def fail(message: str) -> int:
    print(f"error={message}", file=sys.stderr)
    print("validation=FAIL", file=sys.stderr)
    print("status=PR038_VALIDATION_FAILED", file=sys.stderr)
    return 1
...
def main() -> int:
    batch_input = rows(PR038 / "PR038_BATCH_INPUT.csv")
    evidence = rows(PR038 / "PR038_EVIDENCE_MATRIX.csv")
    results = rows(PR038 / "PR038_CLOSURE_RESULTS.csv")
    remaining = rows(PR038 / "PR038_REMAINING_GAPS.csv")
    summary = json.loads(
        (PR038 / "PR038_SUMMARY.json").read_text(encoding="utf-8")
    )
    ...
    if summary["ui_complete_records_created"] != 0:
        return fail("PR-038 must not create UI_COMPLETE records")

    if summary["full_capability_coverage_claimed"] is not False:
        return fail("PR-038 must not claim full capability coverage")
