#!/usr/bin/env python3
"""Validate PR-040 batch-03 evidence closure."""
...
PR040 = ROOT / "docs/99-governance/pr-040-p0-batch-03-evidence-closure"


def fail(message: str) -> int:
    print(f"error={message}", file=sys.stderr)
    print("validation=FAIL", file=sys.stderr)
    print("status=PR040_VALIDATION_FAILED", file=sys.stderr)
    return 1
...
def main() -> int:
    batch_input = rows(PR040 / "PR040_BATCH_INPUT.csv")
    evidence = rows(PR040 / "PR040_EVIDENCE_MATRIX.csv")
    results = rows(PR040 / "PR040_CLOSURE_RESULTS.csv")
    remaining = rows(PR040 / "PR040_REMAINING_GAPS.csv")
    summary = json.loads(
        (PR040 / "PR040_SUMMARY.json").read_text(encoding="utf-8")
    )
    ...
    if summary["ui_complete_records_created"] != 0:
        return fail("PR-040 must not create UI_COMPLETE records")

    if summary["full_capability_coverage_claimed"] is not False:
        return fail("PR-040 must not claim full capability coverage")
    ...
    print("status=PR040_P0_BATCH_01_EVIDENCE_CLOSURE_VALID")
    return 0
