...
const packageDir = path.join(
  root,
  'docs/99-governance/pr-053-p0-batch-15-evidence-closure',
);
const summary = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR053_SUMMARY.json'), 'utf8'),
);
const matrix = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR053_EVIDENCE_MATRIX.json'), 'utf8'),
);
const results = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR053_CLOSURE_RESULTS.json'), 'utf8'),
);

test('PR-053 processes one bounded PR-036 batch', () => {
...
test('PR-053 closure accounting is exact', () => {
  assert.equal(
...
test('PR-053 never closes incomplete evidence', () => {
...
test('PR-053 preserves runtime safety', () => {
