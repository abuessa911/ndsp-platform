...
const packageDir = path.join(
  root,
  'docs/99-governance/pr-051-p0-batch-13-evidence-closure',
);
const summary = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR051_SUMMARY.json'), 'utf8'),
);
const matrix = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR051_EVIDENCE_MATRIX.json'), 'utf8'),
);
const results = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR051_CLOSURE_RESULTS.json'), 'utf8'),
);

test('PR-051 processes one bounded PR-036 batch', () => {
...
test('PR-051 closure accounting is exact', () => {
  assert.equal(
...
test('PR-051 never closes incomplete evidence', () => {
...
test('PR-051 preserves runtime safety', () => {
