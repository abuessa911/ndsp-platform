...
const packageDir = path.join(
  root,
  'docs/99-governance/pr-040-p0-batch-03-evidence-closure',
);
const summary = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR040_SUMMARY.json'), 'utf8'),
);
const matrix = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR040_EVIDENCE_MATRIX.json'), 'utf8'),
);
const results = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR040_CLOSURE_RESULTS.json'), 'utf8'),
);

test('PR-040 processes one bounded PR-036 batch', () => {
...
test('PR-040 closure accounting is exact', () => {
  assert.equal(
...
test('PR-040 never closes incomplete evidence', () => {
...
test('PR-040 preserves runtime safety', () => {
