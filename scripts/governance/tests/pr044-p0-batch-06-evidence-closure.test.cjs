...
const packageDir = path.join(
  root,
  'docs/99-governance/pr-044-p0-batch-06-evidence-closure',
);
const summary = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR044_SUMMARY.json'), 'utf8'),
);
const matrix = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR044_EVIDENCE_MATRIX.json'), 'utf8'),
);
const results = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR044_CLOSURE_RESULTS.json'), 'utf8'),
);

test('PR-044 processes one bounded PR-036 batch', () => {
...
test('PR-044 closure accounting is exact', () => {
  assert.equal(
...
test('PR-044 never closes incomplete evidence', () => {
...
test('PR-044 preserves runtime safety', () => {
