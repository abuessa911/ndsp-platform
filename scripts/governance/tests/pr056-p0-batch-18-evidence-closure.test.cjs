...
const packageDir = path.join(
  root,
  'docs/99-governance/pr-056-p0-batch-18-evidence-closure',
);
const summary = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR056_SUMMARY.json'), 'utf8'),
);
const matrix = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR056_EVIDENCE_MATRIX.json'), 'utf8'),
);
const results = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR056_CLOSURE_RESULTS.json'), 'utf8'),
);

test('PR-056 processes one bounded PR-036 batch', () => {
...
test('PR-056 closure accounting is exact', () => {
  assert.equal(
...
test('PR-056 never closes incomplete evidence', () => {
...
test('PR-056 preserves runtime safety', () => {
