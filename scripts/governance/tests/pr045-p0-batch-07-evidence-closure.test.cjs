...
const packageDir = path.join(
  root,
  'docs/99-governance/pr-045-p0-batch-07-evidence-closure',
);
const summary = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR045_SUMMARY.json'), 'utf8'),
);
const matrix = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR045_EVIDENCE_MATRIX.json'), 'utf8'),
);
const results = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR045_CLOSURE_RESULTS.json'), 'utf8'),
);

test('PR-045 processes one bounded PR-036 batch', () => {
...
test('PR-045 closure accounting is exact', () => {
  assert.equal(
...
test('PR-045 never closes incomplete evidence', () => {
...
test('PR-045 preserves runtime safety', () => {
