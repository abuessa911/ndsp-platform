...
const packageDir = path.join(
  root,
  'docs/99-governance/pr-038-p0-batch-02-evidence-closure',
);
const summary = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR038_SUMMARY.json'), 'utf8'),
);
const matrix = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR038_EVIDENCE_MATRIX.json'), 'utf8'),
);
const results = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR038_CLOSURE_RESULTS.json'), 'utf8'),
);

test('PR-038 processes one bounded PR-036 batch', () => {
...
test('PR-038 closure accounting is exact', () => {
  assert.equal(
...
test('PR-038 never closes incomplete evidence', () => {
...
test('PR-038 preserves runtime safety', () => {
