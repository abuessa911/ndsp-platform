...
const packageDir = path.join(
  root,
  'docs/99-governance/pr-057-p0-batch-19-evidence-closure',
);
const summary = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR057_SUMMARY.json'), 'utf8'),
);
const matrix = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR057_EVIDENCE_MATRIX.json'), 'utf8'),
);
const results = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR057_CLOSURE_RESULTS.json'), 'utf8'),
);

test('PR-057 processes one bounded PR-036 batch', () => {
...
test('PR-057 closure accounting is exact', () => {
  assert.equal(
...
test('PR-057 never closes incomplete evidence', () => {
...
test('PR-057 preserves runtime safety', () => {
