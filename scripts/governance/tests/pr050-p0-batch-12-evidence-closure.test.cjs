...
const packageDir = path.join(
  root,
  'docs/99-governance/pr-050-p0-batch-12-evidence-closure',
);
const summary = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR050_SUMMARY.json'), 'utf8'),
);
const matrix = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR050_EVIDENCE_MATRIX.json'), 'utf8'),
);
const results = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR050_CLOSURE_RESULTS.json'), 'utf8'),
);

test('PR-050 processes one bounded PR-036 batch', () => {
...
test('PR-050 closure accounting is exact', () => {
  assert.equal(
...
test('PR-050 never closes incomplete evidence', () => {
...
test('PR-050 preserves runtime safety', () => {
