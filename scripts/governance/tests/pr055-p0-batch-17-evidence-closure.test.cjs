...
const packageDir = path.join(
  root,
  'docs/99-governance/pr-055-p0-batch-17-evidence-closure',
);
const summary = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR055_SUMMARY.json'), 'utf8'),
);
const matrix = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR055_EVIDENCE_MATRIX.json'), 'utf8'),
);
const results = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR055_CLOSURE_RESULTS.json'), 'utf8'),
);

test('PR-055 processes one bounded PR-036 batch', () => {
...
test('PR-055 closure accounting is exact', () => {
  assert.equal(
...
test('PR-055 never closes incomplete evidence', () => {
...
test('PR-055 preserves runtime safety', () => {
