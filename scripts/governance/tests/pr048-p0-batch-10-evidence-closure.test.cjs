...
const packageDir = path.join(
  root,
  'docs/99-governance/pr-048-p0-batch-10-evidence-closure',
);
const summary = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR048_SUMMARY.json'), 'utf8'),
);
const matrix = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR048_EVIDENCE_MATRIX.json'), 'utf8'),
);
const results = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR048_CLOSURE_RESULTS.json'), 'utf8'),
);

test('PR-048 processes one bounded PR-036 batch', () => {
...
test('PR-048 closure accounting is exact', () => {
  assert.equal(
...
test('PR-048 never closes incomplete evidence', () => {
...
test('PR-048 preserves runtime safety', () => {
