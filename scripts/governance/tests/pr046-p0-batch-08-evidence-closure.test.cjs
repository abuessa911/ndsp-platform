...
const packageDir = path.join(
  root,
  'docs/99-governance/pr-046-p0-batch-08-evidence-closure',
);
const summary = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR046_SUMMARY.json'), 'utf8'),
);
const matrix = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR046_EVIDENCE_MATRIX.json'), 'utf8'),
);
const results = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR046_CLOSURE_RESULTS.json'), 'utf8'),
);

test('PR-046 processes one bounded PR-036 batch', () => {
...
test('PR-046 closure accounting is exact', () => {
  assert.equal(
...
test('PR-046 never closes incomplete evidence', () => {
...
test('PR-046 preserves runtime safety', () => {
