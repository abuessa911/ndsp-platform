...
const packageDir = path.join(
  root,
  'docs/99-governance/pr-052-p0-batch-14-evidence-closure',
);
const summary = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR052_SUMMARY.json'), 'utf8'),
);
const matrix = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR052_EVIDENCE_MATRIX.json'), 'utf8'),
);
const results = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR052_CLOSURE_RESULTS.json'), 'utf8'),
);

test('PR-052 processes one bounded PR-036 batch', () => {
...
test('PR-052 closure accounting is exact', () => {
  assert.equal(
...
test('PR-052 never closes incomplete evidence', () => {
...
test('PR-052 preserves runtime safety', () => {
