...
const packageDir = path.join(
  root,
  'docs/99-governance/pr-042-p0-batch-04-evidence-closure',
);
const summary = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR042_SUMMARY.json'), 'utf8'),
);
const matrix = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR042_EVIDENCE_MATRIX.json'), 'utf8'),
);
const results = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR042_CLOSURE_RESULTS.json'), 'utf8'),
);

test('PR-042 processes one bounded PR-036 batch', () => {
...
test('PR-042 closure accounting is exact', () => {
  assert.equal(
...
test('PR-042 never closes incomplete evidence', () => {
...
test('PR-042 preserves runtime safety', () => {
