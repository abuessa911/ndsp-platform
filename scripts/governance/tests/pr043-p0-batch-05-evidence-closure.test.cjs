...
const packageDir = path.join(
  root,
  'docs/99-governance/pr-043-p0-batch-05-evidence-closure',
);
const summary = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR043_SUMMARY.json'), 'utf8'),
);
const matrix = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR043_EVIDENCE_MATRIX.json'), 'utf8'),
);
const results = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR043_CLOSURE_RESULTS.json'), 'utf8'),
);

test('PR-043 processes one bounded PR-036 batch', () => {
...
test('PR-043 closure accounting is exact', () => {
  assert.equal(
...
test('PR-043 never closes incomplete evidence', () => {
...
test('PR-043 preserves runtime safety', () => {
