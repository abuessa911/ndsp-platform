...
const packageDir = path.join(
  root,
  'docs/99-governance/pr-054-p0-batch-16-evidence-closure',
);
const summary = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR054_SUMMARY.json'), 'utf8'),
);
const matrix = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR054_EVIDENCE_MATRIX.json'), 'utf8'),
);
const results = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR054_CLOSURE_RESULTS.json'), 'utf8'),
);

test('PR-054 processes one bounded PR-036 batch', () => {
...
test('PR-054 closure accounting is exact', () => {
  assert.equal(
...
test('PR-054 never closes incomplete evidence', () => {
...
test('PR-054 preserves runtime safety', () => {
