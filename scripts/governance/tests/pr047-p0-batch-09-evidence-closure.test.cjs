...
const packageDir = path.join(
  root,
  'docs/99-governance/pr-047-p0-batch-09-evidence-closure',
);
const summary = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR047_SUMMARY.json'), 'utf8'),
);
const matrix = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR047_EVIDENCE_MATRIX.json'), 'utf8'),
);
const results = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR047_CLOSURE_RESULTS.json'), 'utf8'),
);

test('PR-047 processes one bounded PR-036 batch', () => {
...
test('PR-047 closure accounting is exact', () => {
  assert.equal(
...
test('PR-047 never closes incomplete evidence', () => {
...
test('PR-047 preserves runtime safety', () => {
