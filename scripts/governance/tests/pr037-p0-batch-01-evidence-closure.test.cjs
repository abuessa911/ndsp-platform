...
const packageDir = path.join(
  root,
  'docs/99-governance/pr-037-p0-batch-01-evidence-closure',
);
const summary = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR037_SUMMARY.json'), 'utf8'),
);
const matrix = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR037_EVIDENCE_MATRIX.json'), 'utf8'),
);
const results = JSON.parse(
  fs.readFileSync(path.join(packageDir, 'PR037_CLOSURE_RESULTS.json'), 'utf8'),
);

test('PR-037 processes one bounded PR-036 batch', () => {
...
test('PR-037 closure accounting is exact', () => {
  assert.equal(
...
test('PR-037 never closes incomplete evidence', () => {
...
test('PR-037 preserves runtime safety', () => {
