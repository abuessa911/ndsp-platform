...
PR035 = ROOT / "docs/99-governance/pr-035-full-coverage-recalculation"
...
def fail(message: str) -> int:
    ...
    print("status=PR035_VALIDATION_FAILED", file=sys.stderr)
    return 1
...
def main() -> int:
    matrix = rows(PR035 / "PR035_CAPABILITY_COVERAGE_MATRIX.csv")
    remaining = rows(PR035 / "PR035_REMAINING_GAPS.csv")
    gap_summary = rows(PR035 / "PR035_GAP_CATEGORY_SUMMARY.csv")
    summary = json.loads((PR035 / "PR035_SUMMARY.json").read_text(encoding="utf-8"))
    ...
    if summary["traceability_rows_modified"] != 0:
        return fail("PR-035 must not modify Traceability")

    if summary["runtime_changes"] != "none":
        return fail("PR-035 must not modify runtime")

    if summary["pr033_closed_capabilities_verified"] != 2:
        return fail("PR-033 closure verification is missing")
    ...
    print("pr033_closure_verification=PASS")
    print("traceability_rows_modified=0")
    print("runtime_changes=none")
    print("validation=PASS")
    print("status=PR035_FULL_COVERAGE_RECALCULATION_VALID")
    return 0
