#!/usr/bin/env python3
"""Validate PR-053 batch-15 evidence closure."""
...
PR053 = ROOT / "docs/99-governance/pr-053-p0-batch-15-evidence-closure"


def fail(message: str) -> int:
    print(f"error={message}", file=sys.stderr)
    print("validation=FAIL", file=sys.stderr)
    print("status=PR053_VALIDATION_FAILED", file=sys.stderr)
    return 1
...
def main() -> int:
    batch_input = rows(PR053 / "PR053_BATCH_INPUT.csv")
    evidence = rows(PR053 / "PR053_EVIDENCE_MATRIX.csv")
    results = rows(PR053 / "PR053_CLOSURE_RESULTS.csv")
    remaining = rows(PR053 / "PR053_REMAINING_GAPS.csv")
    summary = json.loads(
        (PR053 / "PR053_SUMMARY.json").read_text(encoding="utf-8")
    )
    ...
    if summary["ui_complete_records_created"] != 0:
        return fail("PR-053 must not create UI_COMPLETE records")

    if summary["full_capability_coverage_claimed"] is not False:
        return fail("PR-053 must not claim full capability coverage")
    ...
    print("status=PR053_P0_BATCH_15_EVIDENCE_CLOSURE_VALID")
    return 0
