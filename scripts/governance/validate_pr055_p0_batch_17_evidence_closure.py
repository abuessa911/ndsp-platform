#!/usr/bin/env python3
"""Validate PR-055 batch-17 evidence closure."""
...
PR055 = ROOT / "docs/99-governance/pr-055-p0-batch-17-evidence-closure"


def fail(message: str) -> int:
    print(f"error={message}", file=sys.stderr)
    print("validation=FAIL", file=sys.stderr)
    print("status=PR055_VALIDATION_FAILED", file=sys.stderr)
    return 1
...
def main() -> int:
    batch_input = rows(PR055 / "PR055_BATCH_INPUT.csv")
    evidence = rows(PR055 / "PR055_EVIDENCE_MATRIX.csv")
    results = rows(PR055 / "PR055_CLOSURE_RESULTS.csv")
    remaining = rows(PR055 / "PR055_REMAINING_GAPS.csv")
    summary = json.loads(
        (PR055 / "PR055_SUMMARY.json").read_text(encoding="utf-8")
    )
    ...
    if summary["ui_complete_records_created"] != 0:
        return fail("PR-055 must not create UI_COMPLETE records")

    if summary["full_capability_coverage_claimed"] is not False:
        return fail("PR-055 must not claim full capability coverage")
    ...
    print("status=PR055_P0_BATCH_17_EVIDENCE_CLOSURE_VALID")
    return 0
