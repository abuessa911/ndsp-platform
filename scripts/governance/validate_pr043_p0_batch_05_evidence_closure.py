#!/usr/bin/env python3
"""Validate PR-043 batch-05 evidence closure."""
...
PR043 = ROOT / "docs/99-governance/pr-043-p0-batch-05-evidence-closure"


def fail(message: str) -> int:
    print(f"error={message}", file=sys.stderr)
    print("validation=FAIL", file=sys.stderr)
    print("status=PR043_VALIDATION_FAILED", file=sys.stderr)
    return 1
...
def main() -> int:
    batch_input = rows(PR043 / "PR043_BATCH_INPUT.csv")
    evidence = rows(PR043 / "PR043_EVIDENCE_MATRIX.csv")
    results = rows(PR043 / "PR043_CLOSURE_RESULTS.csv")
    remaining = rows(PR043 / "PR043_REMAINING_GAPS.csv")
    summary = json.loads(
        (PR043 / "PR043_SUMMARY.json").read_text(encoding="utf-8")
    )
    ...
    if summary["ui_complete_records_created"] != 0:
        return fail("PR-043 must not create UI_COMPLETE records")

    if summary["full_capability_coverage_claimed"] is not False:
        return fail("PR-043 must not claim full capability coverage")
    ...
    print("status=PR043_P0_BATCH_05_EVIDENCE_CLOSURE_VALID")
    return 0
