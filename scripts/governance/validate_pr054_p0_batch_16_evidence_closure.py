#!/usr/bin/env python3
"""Validate PR-054 batch-16 evidence closure."""
...
PR054 = ROOT / "docs/99-governance/pr-054-p0-batch-16-evidence-closure"


def fail(message: str) -> int:
    print(f"error={message}", file=sys.stderr)
    print("validation=FAIL", file=sys.stderr)
    print("status=PR054_VALIDATION_FAILED", file=sys.stderr)
    return 1
...
def main() -> int:
    batch_input = rows(PR054 / "PR054_BATCH_INPUT.csv")
    evidence = rows(PR054 / "PR054_EVIDENCE_MATRIX.csv")
    results = rows(PR054 / "PR054_CLOSURE_RESULTS.csv")
    remaining = rows(PR054 / "PR054_REMAINING_GAPS.csv")
    summary = json.loads(
        (PR054 / "PR054_SUMMARY.json").read_text(encoding="utf-8")
    )
    ...
    if summary["ui_complete_records_created"] != 0:
        return fail("PR-054 must not create UI_COMPLETE records")

    if summary["full_capability_coverage_claimed"] is not False:
        return fail("PR-054 must not claim full capability coverage")
    ...
    print("status=PR054_P0_BATCH_16_EVIDENCE_CLOSURE_VALID")
    return 0
