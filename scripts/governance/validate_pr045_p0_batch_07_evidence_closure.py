#!/usr/bin/env python3
"""Validate PR-045 batch-07 evidence closure."""
...
PR045 = ROOT / "docs/99-governance/pr-045-p0-batch-07-evidence-closure"


def fail(message: str) -> int:
    print(f"error={message}", file=sys.stderr)
    print("validation=FAIL", file=sys.stderr)
    print("status=PR045_VALIDATION_FAILED", file=sys.stderr)
    return 1
...
def main() -> int:
    batch_input = rows(PR045 / "PR045_BATCH_INPUT.csv")
    evidence = rows(PR045 / "PR045_EVIDENCE_MATRIX.csv")
    results = rows(PR045 / "PR045_CLOSURE_RESULTS.csv")
    remaining = rows(PR045 / "PR045_REMAINING_GAPS.csv")
    summary = json.loads(
        (PR045 / "PR045_SUMMARY.json").read_text(encoding="utf-8")
    )
    ...
    if summary["ui_complete_records_created"] != 0:
        return fail("PR-045 must not create UI_COMPLETE records")

    if summary["full_capability_coverage_claimed"] is not False:
        return fail("PR-045 must not claim full capability coverage")
    ...
    print("status=PR045_P0_BATCH_07_EVIDENCE_CLOSURE_VALID")
    return 0
