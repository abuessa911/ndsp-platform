#!/usr/bin/env python3
"""Validate PR-046 batch-08 evidence closure."""
...
PR046 = ROOT / "docs/99-governance/pr-046-p0-batch-08-evidence-closure"


def fail(message: str) -> int:
    print(f"error={message}", file=sys.stderr)
    print("validation=FAIL", file=sys.stderr)
    print("status=PR046_VALIDATION_FAILED", file=sys.stderr)
    return 1
...
def main() -> int:
    batch_input = rows(PR046 / "PR046_BATCH_INPUT.csv")
    evidence = rows(PR046 / "PR046_EVIDENCE_MATRIX.csv")
    results = rows(PR046 / "PR046_CLOSURE_RESULTS.csv")
    remaining = rows(PR046 / "PR046_REMAINING_GAPS.csv")
    summary = json.loads(
        (PR046 / "PR046_SUMMARY.json").read_text(encoding="utf-8")
    )
    ...
    if summary["ui_complete_records_created"] != 0:
        return fail("PR-046 must not create UI_COMPLETE records")

    if summary["full_capability_coverage_claimed"] is not False:
        return fail("PR-046 must not claim full capability coverage")
    ...
    print("status=PR046_P0_BATCH_08_EVIDENCE_CLOSURE_VALID")
    return 0
