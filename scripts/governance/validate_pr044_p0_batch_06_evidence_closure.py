#!/usr/bin/env python3
"""Validate PR-044 batch-06 evidence closure."""
...
PR044 = ROOT / "docs/99-governance/pr-044-p0-batch-06-evidence-closure"


def fail(message: str) -> int:
    print(f"error={message}", file=sys.stderr)
    print("validation=FAIL", file=sys.stderr)
    print("status=PR044_VALIDATION_FAILED", file=sys.stderr)
    return 1
...
def main() -> int:
    batch_input = rows(PR044 / "PR044_BATCH_INPUT.csv")
    evidence = rows(PR044 / "PR044_EVIDENCE_MATRIX.csv")
    results = rows(PR044 / "PR044_CLOSURE_RESULTS.csv")
    remaining = rows(PR044 / "PR044_REMAINING_GAPS.csv")
    summary = json.loads(
        (PR044 / "PR044_SUMMARY.json").read_text(encoding="utf-8")
    )
    ...
    if summary["ui_complete_records_created"] != 0:
        return fail("PR-044 must not create UI_COMPLETE records")

    if summary["full_capability_coverage_claimed"] is not False:
        return fail("PR-044 must not claim full capability coverage")
    ...
    print("status=PR044_P0_BATCH_06_EVIDENCE_CLOSURE_VALID")
    return 0
