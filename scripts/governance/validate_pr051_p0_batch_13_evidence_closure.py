#!/usr/bin/env python3
"""Validate PR-051 batch-13 evidence closure."""
...
PR051 = ROOT / "docs/99-governance/pr-051-p0-batch-13-evidence-closure"


def fail(message: str) -> int:
    print(f"error={message}", file=sys.stderr)
    print("validation=FAIL", file=sys.stderr)
    print("status=PR051_VALIDATION_FAILED", file=sys.stderr)
    return 1
...
def main() -> int:
    batch_input = rows(PR051 / "PR051_BATCH_INPUT.csv")
    evidence = rows(PR051 / "PR051_EVIDENCE_MATRIX.csv")
    results = rows(PR051 / "PR051_CLOSURE_RESULTS.csv")
    remaining = rows(PR051 / "PR051_REMAINING_GAPS.csv")
    summary = json.loads(
        (PR051 / "PR051_SUMMARY.json").read_text(encoding="utf-8")
    )
    ...
    if summary["ui_complete_records_created"] != 0:
        return fail("PR-051 must not create UI_COMPLETE records")

    if summary["full_capability_coverage_claimed"] is not False:
        return fail("PR-051 must not claim full capability coverage")
    ...
    print("status=PR051_P0_BATCH_13_EVIDENCE_CLOSURE_VALID")
    return 0
