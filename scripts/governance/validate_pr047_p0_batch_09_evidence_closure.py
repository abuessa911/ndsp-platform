#!/usr/bin/env python3
"""Validate PR-047 batch-09 evidence closure."""
...
PR047 = ROOT / "docs/99-governance/pr-047-p0-batch-09-evidence-closure"


def fail(message: str) -> int:
    print(f"error={message}", file=sys.stderr)
    print("validation=FAIL", file=sys.stderr)
    print("status=PR047_VALIDATION_FAILED", file=sys.stderr)
    return 1
...
def main() -> int:
    batch_input = rows(PR047 / "PR047_BATCH_INPUT.csv")
    evidence = rows(PR047 / "PR047_EVIDENCE_MATRIX.csv")
    results = rows(PR047 / "PR047_CLOSURE_RESULTS.csv")
    remaining = rows(PR047 / "PR047_REMAINING_GAPS.csv")
    summary = json.loads(
        (PR047 / "PR047_SUMMARY.json").read_text(encoding="utf-8")
    )
    ...
    if summary["ui_complete_records_created"] != 0:
        return fail("PR-047 must not create UI_COMPLETE records")

    if summary["full_capability_coverage_claimed"] is not False:
        return fail("PR-047 must not claim full capability coverage")
    ...
    print("status=PR047_P0_BATCH_09_EVIDENCE_CLOSURE_VALID")
    return 0
