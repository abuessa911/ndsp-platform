#!/usr/bin/env python3
"""Validate PR-052 batch-14 evidence closure."""
...
PR052 = ROOT / "docs/99-governance/pr-052-p0-batch-14-evidence-closure"


def fail(message: str) -> int:
    print(f"error={message}", file=sys.stderr)
    print("validation=FAIL", file=sys.stderr)
    print("status=PR052_VALIDATION_FAILED", file=sys.stderr)
    return 1
...
def main() -> int:
    batch_input = rows(PR052 / "PR052_BATCH_INPUT.csv")
    evidence = rows(PR052 / "PR052_EVIDENCE_MATRIX.csv")
    results = rows(PR052 / "PR052_CLOSURE_RESULTS.csv")
    remaining = rows(PR052 / "PR052_REMAINING_GAPS.csv")
    summary = json.loads(
        (PR052 / "PR052_SUMMARY.json").read_text(encoding="utf-8")
    )
    ...
    if summary["ui_complete_records_created"] != 0:
        return fail("PR-052 must not create UI_COMPLETE records")

    if summary["full_capability_coverage_claimed"] is not False:
        return fail("PR-052 must not claim full capability coverage")
    ...
    print("status=PR052_P0_BATCH_14_EVIDENCE_CLOSURE_VALID")
    return 0
