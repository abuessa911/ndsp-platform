#!/usr/bin/env python3
"""Validate PR-049 batch-11 evidence closure."""
...
PR049 = ROOT / "docs/99-governance/pr-049-p0-batch-11-evidence-closure"


def fail(message: str) -> int:
    print(f"error={message}", file=sys.stderr)
    print("validation=FAIL", file=sys.stderr)
    print("status=PR049_VALIDATION_FAILED", file=sys.stderr)
    return 1
...
def main() -> int:
    batch_input = rows(PR049 / "PR049_BATCH_INPUT.csv")
    evidence = rows(PR049 / "PR049_EVIDENCE_MATRIX.csv")
    results = rows(PR049 / "PR049_CLOSURE_RESULTS.csv")
    remaining = rows(PR049 / "PR049_REMAINING_GAPS.csv")
    summary = json.loads(
        (PR049 / "PR049_SUMMARY.json").read_text(encoding="utf-8")
    )
    ...
    if summary["ui_complete_records_created"] != 0:
        return fail("PR-049 must not create UI_COMPLETE records")

    if summary["full_capability_coverage_claimed"] is not False:
        return fail("PR-049 must not claim full capability coverage")
    ...
    print("status=PR049_P0_BATCH_11_EVIDENCE_CLOSURE_VALID")
    return 0
